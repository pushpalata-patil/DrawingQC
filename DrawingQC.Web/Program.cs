...
app.MapPost("/api/sync-autocad", async (HttpRequest request) =>
{
    ...
    if (!OperatingSystem.IsWindows())
        return Results.Problem("AutoCAD sync runs through the DrawingQC Local Agent on your PC. Start the agent (download it from the sidebar) with AutoCAD open, then click Sync again.");
...
app.MapPost("/api/booklet", async (HttpRequest request) =>
{
    if (CurrentUser(request) == null)
        return Results.Json(new { error = "Please sign in first." }, statusCode: 401);
    if (!OperatingSystem.IsWindows())
        return Results.Problem("Booklet generation needs Microsoft Word, so it runs through the DrawingQC Local Agent on your PC. Start the agent (download it from the sidebar), then click Generate again.");
    if (!request.HasFormContentType)
...
// A booklet generated by the Local Agent (on the user's PC) reports back here so the run still
// lands in the shared history.
app.MapPost("/api/booklet/record", (HttpRequest request, BookletRecordDto dto) =>
{
    var user = CurrentUser(request);
    if (user == null) return Results.Json(new { error = "Please sign in first." }, statusCode: 401);
    DrawingQC.Web.Db.RecordBookletRun(user.Id, dto.template, dto.excel, dto.drawings, dto.bom, dto.output, dto.rev, dto.date, dto.sizeMB);
    return Results.Ok(new { ok = true });
});

// ---------- Local Agent (Windows companion for AutoCAD / Plant 3D + Word) ----------
// The browser talks to the agent directly on localhost; the server only tells the page where to
// find it and where the installer zip is (built into the image by the Dockerfile).
app.MapGet("/api/agent/info", () =>
{
    var root = app.Environment.WebRootPath ?? Path.Combine(AppContext.BaseDirectory, "wwwroot");
    var zip = Path.Combine(root, "agent", "DrawingQC.Agent.zip");
    return Results.Ok(new
    {
        port = DrawingQC.Web.AgentInfo.DefaultPort,
        downloadUrl = File.Exists(zip) ? "/agent/DrawingQC.Agent.zip" : null,
        serverIsWindows = OperatingSystem.IsWindows(),
    });
});
...
record BookletRecordDto(string? template, string? excel, string? drawings, string? bom, string? output, string? rev, string? date, double sizeMB);
